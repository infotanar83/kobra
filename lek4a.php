...
$sql = "SELECT projekt_nev, projekt_tipus, tipus_muvelet, projekt_kod, kezdes, vege, homokszoros, festo, elokeszito FROM optmelo";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
  // output data of each row
  while($row = $result->fetch_assoc()) {
    echo "id: " . $row["projekt_nev"]. " - Name: " . $row["projekt_tipus"]. " " . "<br>";
  $pr_n[$i]=$row["projekt_nev"];
  $pr_t[$i]=$row["projekt_tipus"];
  $tip_muv[$i]=$row["tipus_muvelet"];
  $projekt_kod[$i]=$row["projekt_kod"];
   $kezd[$i]=$row["kezdes"];
   $veg[$i]=$row["vege"];
   $homok[$i]=$row["homokszoros"];
   $fest[$i]=$row["festo"];
   $elokeszit[$i]=$row["elokeszito"];
  $i++;
  }
} else {
  echo "0 results";
...
var chart = new CanvasJS.Chart("chartContainer", {
	...
	data: [{
		...
		dataPoints: [
			...
			{ label: "Australia", y: <?php echo json_encode($pr_n[2],
							JSON_NUMERIC_CHECK); ?>, gdp: <?php echo json_encode($pr_t[2],
